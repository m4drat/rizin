...
bool test_simple_macros(void) {
	...
	const RzCmdMacro *macro1 = rz_list_head(l);
...
static RzCmdStatus b_handler(RzCore *core, int argc, const char **argv) {
	...
	if (!strcmp(argv[1], "10")) {
		return !strcmp(argv[2], "20") ? RZ_CMD_STATUS_OK : RZ_CMD_STATUS_ERROR;
	} else if (!strcmp(argv[1], "30")) {
		return !strcmp(argv[2], "40") ? RZ_CMD_STATUS_OK : RZ_CMD_STATUS_ERROR;
	}
	return RZ_CMD_STATUS_ERROR;
}
...
bool test_call_macros(void) {
	RzCmd *cmd = rz_cmd_new(NULL, false);
	RzCmdDesc *root = rz_cmd_get_root(cmd);
	RzCmdDesc *a_cd = rz_cmd_desc_argv_new(cmd, root, "a", a_handler, &fake_help);
	RzCmdDescArg b_args[] = {
	...
	RzCmdDesc *b_cd = rz_cmd_desc_argv_new(cmd, root, "b", b_handler, &b_help);
